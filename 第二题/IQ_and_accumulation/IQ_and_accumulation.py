...
for i in range(n_examples):
    ...
    moment_1_8.append(self_moments(comp_data,[1,3,5,7]))
...
extraData = [Param_R,M_1,M_2,M_3,M_4]
extraData_np = np.array(extraData).reshape((220000,5))
...
filepath3 = './IQ_and_accumulation/IQ_and_accumulation.wts.h5'
# Set up some params
...
plt.grid(True)
plt.legend()
plt.savefig("./IQ_and_accumulation/loss", format='png', dpi=300)
...
plt.savefig("./IQ_and_accumulation/confu_matrix_total",format='png', dpi=300)
acc = {}
...
for snr in snrs:
    ...
    plt.savefig("./IQ_and_accumulation/comf_Matrix_for_snr=" + str(snr), format='png', dpi=300)  # 设置 dpi 参数以调整保存的图像质量
    #   拿到原始混淆矩阵对角线的元素并求和
...
print(acc)
fd = open('./IQ_and_accumulation/acc_trend.dat', 'wb')
...
plt.grid(True)
plt.savefig("./IQ_and_accumulation/acc_trend", format='png', dpi=300)  # 设置 dpi 参数以调整保存的图像质量