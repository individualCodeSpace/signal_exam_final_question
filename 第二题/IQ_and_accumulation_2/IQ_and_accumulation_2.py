...
extraData = [Param_R,M_1,M_2,M_3,M_4,np.real(C_60),np.imag(C_60),np.real(C_63),np.imag(C_63),np.real(C_80),np.imag(C_80)]
extraData_same = [np.array(item).flatten() for item in extraData]
# 将所有子数组堆叠为一个二维数组
extraData_np = np.array(extraData_same).reshape((220000,11))
...
filepath3 = './IQ_and_accumulation_2/IQ_and_accumulation_2.wts.h5'
# Set up some params
...
plt.grid(True)
plt.legend()
plt.savefig("./IQ_and_accumulation_3/loss", format='png', dpi=300)
...
plt.savefig("./IQ_and_accumulation_3/confu_matrix_total",format='png', dpi=300)
acc = {}
...
for snr in snrs:
    ...
    plt.savefig("./IQ_and_accumulation_3/comf_Matrix_for_snr=" + str(snr), format='png', dpi=300)  # 设置 dpi 参数以调整保存的图像质量
    #   拿到原始混淆矩阵对角线的元素并求和
...
print(acc)
fd = open('./IQ_and_accumulation_3/acc_trend.dat', 'wb')
...
plt.grid(True)
plt.savefig("./IQ_and_accumulation_3/acc_trend", format='png', dpi=300)  # 设置 dpi 参数以调整保存的图像质量